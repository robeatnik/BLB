...
def input_file(df):
    ...
    cutoff = FLAGS.data_cutoff
    # No cutoff, 1 key, N-1 features
    if cutoff < 0:
        cutoff = len(data[0]) - 1
    return data[:, 0], data[:, 1:cutoff + 1]
...
def prepare_train_data(data, nPixels, nLabels, size):
    ...
    x_validate = np.array(
        [i[:nPixels:] for i in data_all[size_train:mark_validate]])
    y_validate = np.array(
        [i[nPixels::] for i in data_all[size_train:mark_validate]])

    mark_test = mark_validate + size_test
    x_test = np.array(
        [i[:nPixels:] for i in data_all[mark_validate:mark_test]])
    y_test = np.array(
        [i[nPixels::] for i in data_all[mark_validate:mark_test]])
...
def train(train_data, train_param):
    ...
    # L2-regularization
    #
    l2_loss = tf.nn.l2_loss(W1) + tf.nn.l2_loss(b1) + tf.nn.l2_loss(
        W2) + tf.nn.l2_loss(b2)
    cross_entropy = tf.reduce_mean(
        tf.nn.sigmoid_cross_entropy_with_logits(labels=y_, logits=y))
    regularized_cross_entropy = cross_entropy + beta * l2_loss
    train_step = tf.train.GradientDescentOptimizer(rLearn).minimize(
        regularized_cross_entropy)
    ...
    with tf.Session() as sess:
        ...
        for nstep in range(nSteps * tCycles):
            ...
            if (nstep + 1) % fValidate == 0:
                temp_acc = sess.run(
                    accuracy, feed_dict={x: x_validate,
                                         y_: y_validate})
                temp_ce = sess.run(
                    cross_entropy, feed_dict={x: batch_xs[i],
                                              y_: batch_ys[i]})
                #        temp_l2 = sess.run(l2_loss, feed_dict={x: batch_xs[i], y_: batch_ys[i]})
                temp_l2 = sess.run(l2_loss)
                print(hc, nstep + 1, temp_acc, temp_ce, temp_l2,
                      temp_ce / temp_l2, beta, rLearn)

        # Test
        print('# Train result:', hc,
              sess.run(accuracy, feed_dict={x: x_train,
                                            y_: y_train}))
        print('# Test result:', hc,
              sess.run(accuracy, feed_dict={x: x_test,
                                            y_: y_test}))

        # Analyze at criticality
        if len(x_critical) != 0:
            print('# Critical result:', hc,
                  sess.run(
                      accuracy, feed_dict={x: x_critical,
                                           y_: y_critical}))


def main(_):
    # Import data
    key, data_all = input_file(FLAGS.data_file)
    ...
    hInterval = hw
    hWidth = FLAGS.h_range
    #  hLowerBound=hmin+hWidth
    hLowerBound = hmin
    #  hUpperBound=hmax-hWidth
    hUpperBound = hmax
    ...
    for i in range(nInterval + 1):
        ...
        # This part is model dependent! Only works for [0,2], in units of pi
        if hmin < 0.:
            data1, label1 = set_label(key, data_all, hmin + 2., hc + 2., 2.)
            data = np.vstack((data, data1))
            label = np.vstack((label, label1))
        elif hmax > 2.:
            data1, label1 = set_label(key, data_all, 0., hc - 2., hmax - 2.)
            data = np.vstack((data, data1))
            label = np.vstack((label, label1))
        ...
        train_data = (data[:size_train], label[:size_train],
                      ...
                      data[:size_test], label[:size_test], nullArray,
                      nullArray)
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        type=int,
        default=-1,
        help='number of features to use (default -1, no cutoff)')
    parser.add_argument(
        '--h_min', type=float, default=0.0, help='lower bound of h')
    parser.add_argument(
        '--h_max', type=float, default=2.0, help='upper bound of h')
    parser.add_argument(
        '--h_step', type=float, default=0.03125, help='step of h')
    parser.add_argument(
        '--h_range', type=float, default=0.25, help='range of h (single side)')
    parser.add_argument(
        '--training_steps',
        type=int,
        default=5000,
    ...
    print('# ', ' '.join(sys.argv)) # subarashiisenseidesune!!!!
    tf.app.run()
